...
pub(crate) struct SizeClasses<'a>(
    pub(crate) [imp::Exclusive<Ptr<FreeCell<'a>>>; SizeClasses::NUM_SIZE_CLASSES],
);
...
impl<'a> SizeClasses<'a> {
    ...
    pub(crate) fn get(&self, size: Words) -> Option<&imp::Exclusive<Ptr<FreeCell<'a>>>> {
...
pub(crate) struct SizeClassAllocPolicy<'a, 'b>(pub(crate) &'b imp::Exclusive<Ptr<FreeCell<'a>>>)
where
    'a: 'b;
