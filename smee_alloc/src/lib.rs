...
`smee_alloc`: The **S**hared-**M**emory-**E**nabled, **E**lfin Allocator.
...
- **Elfin, i.e. small:** Generates less than a kilobyte of uncompressed
  ...
  infrastructure. `smee_alloc` won't bloat your `.wasm` download size on the Web.

- **Shared Memory enabled:** Designed for cross process ptr de-referencing.

`smee_alloc` is focused on targeting WebAssembly, producing a small `.wasm` code
...
for small code size. In contrast, `smee_alloc` would be a poor choice for a
scenario where allocation is a performance bottleneck.

Although WebAssembly is the primary target, `smee_alloc` also has an `mmap` based
...
testing `smee_alloc`, and code using `smee_alloc`, without a browser or
WebAssembly engine.

`smee_alloc` compiles on stable Rust 1.33 and newer.

- [Using `smee_alloc` as the Global Allocator](#using-smee_alloc-as-the-global-allocator)
- [`cargo` Features](#cargo-features)
- [Implementation Notes and Constraints](#implementation-notes-and-constraints)
- [License](#license)
- [Contribution](#contribution)

## Using `smee_alloc` as the Global Allocator

```
extern crate smee_alloc;

// Use `smee_alloc` as the global allocator.
#[global_allocator]
static ALLOC: smee_alloc::SmeeAlloc = smee_alloc::SmeeAlloc::INIT;
# fn main() {}
```
...
- **extra_assertions**: Enable various extra, expensive integrity assertions and
  ...
  runtime overhead. It is useful when debugging a use-after-free or `smee_alloc`
  itself.
...
- **static_array_backend**: Force the use of an OS-independent backing
  ...
  `smee_ALLOC_STATIC_ARRAY_BACKEND_BYTES`. Note that this feature requires
  nightly Rust.
...
- `smee_alloc` imposes two words of overhead on each allocation for maintaining
  its internal free lists.

- Deallocation is an *O(1)* operation.

- `smee_alloc` will never return freed pages to the WebAssembly engine /
  operating system. Currently, WebAssembly can only grow its heap, and can never
  shrink it. All allocated pages are indefinitely kept in `smee_alloc`'s internal
  free lists for potential future allocations, even when running on unix
  targets.

- `smee_alloc` uses a simple, first-fit free list implementation. This means that
...
Finally, here is a diagram giving an overview of `smee_alloc`'s implementation:
...
See
[CONTRIBUTING.md](https://github.com/rustwasm/smee_alloc/blob/master/CONTRIBUTING.md)
for hacking!
...
cfg_if! {
    if #[cfg(feature = "static_array_backend")] {
        ...
        compile_error! {
            "There is no `smee_alloc` implementation for this target; want to send a pull request? :)"
        }
    }
}
...
struct FreeCell<'a> {
    header: CellHeader<'a>,
    next_free_raw: Cell<Ptr<FreeCell<'a>>>,
}
...
impl<'a> CellHeader<'a> {
    ...
    // use-after-free, or (b) there is a bug in `smee_alloc` and its size classes
    // implementation.
    #[cfg(feature = "size_classes")]
    ...
    // use-after-free, or (b) there is a bug in `smee_alloc` and its main free
    // list implementation.
...
impl<'a> FreeCell<'a> {
    ...
    fn next_free_can_merge(&self) -> bool {
        self.next_free_raw.get().as_ptr() as usize & Self::NEXT_FREE_CELL_CAN_MERGE != 0
    }

    fn set_next_free_can_merge(&self) {
        let next_free = self.next_free_raw.get().as_ptr() as usize;
        let next_free = next_free | Self::NEXT_FREE_CELL_CAN_MERGE;
        self.next_free_raw
            .set(Ptr::from(next_free as *const FreeCell));
    }
    ...
    fn next_free(&self) -> Ptr<FreeCell<'a>> {
        let next_free = self.next_free_raw.get().as_ptr() as usize & Self::MASK;
        Ptr::from(next_free as *const FreeCell<'a>)
    }

    unsafe fn from_uninitialized(
        raw: NonNull<u8>,
        size: Bytes,
        next_free: Option<Ptr<FreeCell<'a>>>,
        policy: &dyn AllocPolicy<'a>,
    ) -> Ptr<FreeCell<'a>> {
        assert_is_word_aligned(raw.as_ptr() as *mut u8);

        let next_free = next_free.unwrap_or(Ptr::dangling());
    ...
    fn try_alloc<'b>(
        &'b self,
        previous: &'b Cell<Ptr<FreeCell<'a>>>,
        alloc_size: Words,
        align: Bytes,
        policy: &dyn AllocPolicy<'a>,
        ...
        if data + size_of::<CellHeader>().0 + min_cell_size.0 <= split_and_aligned {
            ...
            let split_cell: Ptr<FreeCell<'a>> = unsafe {
            ...
            let x = split_cell.as_ptr();
            unsafe {
                return Some((&*x).into_allocated_cell(policy));
            }
        }
        ...
        if self.header.is_aligned_to(align) {
            ...
            assert_is_valid_free_list(previous.get().as_ptr(), policy);
    ...
    fn insert_into_free_list<'b>(
        &'b self,
        head: &'b Cell<Ptr<FreeCell<'a>>>,
        policy: &dyn AllocPolicy<'a>,
    ) -> &'b Cell<Ptr<FreeCell<'a>>> {
        extra_assert!(!self.next_free_can_merge());
        extra_assert!(self.next_free().is_null());
        self.next_free_raw.set(head.get());
        head.set(Ptr::from(self as *const _));
        assert_is_valid_free_list(head.get().as_ptr(), policy);
        head
    }
...
impl<'a> AllocatedCell<'a> {
    unsafe fn into_free_cell(&self, policy: &dyn AllocPolicy<'a>) -> &FreeCell<'a> {
        ...
        free.next_free_raw.set(Ptr::dangling());
        free
    }
            ...
            if let Some(cell_ref) = cell.as_ref() {
                ...
                if let Some(free) = cell_ref.as_free_cell() {
                    if free.next_free_can_merge() {
                        ...
                        assert_eq!(
                            free.next_free().as_ptr() as *const CellHeader,
...
extra_only! {
    ...
    fn assert_is_valid_free_list(head: *const FreeCell, policy: &dyn AllocPolicy) {
        unsafe {
            ...
            let mut right = (*left).next_free().as_ptr();
...
trait AllocPolicy<'a> {
    unsafe fn new_cell_for_free_list(
        ...
        size: Words,
        align: Bytes,
    ) -> Result<Ptr<FreeCell<'a>>, AllocErr>;
...
impl<'a> AllocPolicy<'a> for LargeAllocPolicy {
    unsafe fn new_cell_for_free_list(
        ...
        Ok(Ptr::from(free_cell as *const _))
    }
...
unsafe fn walk_free_list<'a, F, T>(
    head: &Cell<Ptr<FreeCell<'a>>>,
    policy: &dyn AllocPolicy<'a>,
    mut f: F,
) -> Result<T, AllocErr>
where
    F: FnMut(&Cell<Ptr<FreeCell<'a>>>, &FreeCell<'a>) -> Option<T>,
{
    ...
    loop {
        ...
        while (*current_free.get()).next_free_can_merge() {
            ...
            previous_free.set(Ptr::from(prev_neighbor as *const _));
            current_free.set(Ptr::from(prev_neighbor as *const _));
        ...
        previous_free.set((*current_free.get().as_ptr()).next_free_raw.get());
    }
}
...
unsafe fn alloc_first_fit<'a>(
    ...
    walk_free_list(head, policy, |previous, current| {
        extra_assert_eq!(previous.get().as_ptr(), current as *const _);
...
pub struct SmeeAlloc<'a> {
    head: imp::Exclusive<Ptr<FreeCell<'a>>>,

    #[cfg(feature = "size_classes")]
    size_classes: size_classes::SizeClasses<'a>,
}

unsafe impl<'a> Sync for SmeeAlloc<'a> {}

impl<'a> ConstInit for SmeeAlloc<'a> {
    const INIT: SmeeAlloc<'a> = SmeeAlloc {
...
impl<'a> SmeeAlloc<'a> {
    /// An initial `const` default construction of a `SmeeAlloc` allocator.
    ///
    ...
    where
        F: for<'b> FnOnce(&'b Cell<Ptr<FreeCell<'a>>>, &'b dyn AllocPolicy<'a>) -> T,
    {
        extra_assert!(size.0 > 0);
        extra_assert!(align.0 > 0);
    ...
    unsafe fn dealloc_impl(&self, ptr: NonNull<u8>, layout: Layout) {
        ...
        self.with_free_list_and_policy_for_size(size, align, |head, policy| {
            ...
            // todo(dillybar): SOUNDNESS? ptr -> ref -> ptr
            ...
            if policy.should_merge_adjacent_free_cells() {
                    ...
                    .and_then(|n| (*n).as_free_cell())
                {
                    ...
                    next.next_free_raw.set(Ptr::from(free as *const _));
...
#[cfg(feature = "nightly")]
unsafe impl<'a, 'b> Alloc for &'b SmeeAlloc<'a>
where
    'a: 'b,
{
...
unsafe impl GlobalAlloc for SmeeAlloc<'static> {
