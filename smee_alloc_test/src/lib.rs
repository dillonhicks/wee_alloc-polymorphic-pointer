...
extern crate cfg_if;
extern crate rand;
extern crate smee_alloc as smee_alloc;
...
impl Operation {
    ...
    fn arbitrary_alloc<G: Gen>(
        ...
        *num_allocs += 1;
        ...
        // XXX: Keep this synced with `smee_alloc`.
...
impl Operations {
    pub fn run_single_threaded(&self) {
        self.run_with_allocator(&smee_alloc::SmeeAlloc::INIT);
    }

    pub fn run_multi_threaded(ops0: Self, ops1: Self, ops2: Self, ops3: Self) {
        use std::thread;

        static SMEE: smee_alloc::SmeeAlloc = smee_alloc::SmeeAlloc::INIT;

        let handle0 = thread::spawn(move || ops0.run_with_allocator(&SMEE));
        let handle1 = thread::spawn(move || ops1.run_with_allocator(&SMEE));
        let handle2 = thread::spawn(move || ops2.run_with_allocator(&SMEE));
        let handle3 = thread::spawn(move || ops3.run_with_allocator(&SMEE));
...
quickcheck! {
    fn single_allocation_with_size_and_align(size: usize, align: usize) -> () {
        ...
        let mut w = &smee_alloc::SmeeAlloc::INIT;
...
#[test]
fn smoke() {
    let mut a = &smee_alloc::SmeeAlloc::INIT;
    unsafe {
